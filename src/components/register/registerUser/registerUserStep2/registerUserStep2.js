...
const RegisterUserStep2 = ({userData}) => {
    ...
    const [selectedTimeEdu,setSelectedTimeEdu] = useState(1)
    ...
    const onSubmit = (e) => {
        ...
        const newData = {
            ...
            studyLang: studyLang,
            selectedSubjects,
            time: times.filter(item => item.id === +selectedTimeEdu)[0].name
        }



        setPostDataStatus(true)
        request(`${BackUrl}register`, "POST", JSON.stringify(newData))
            .then(res => {
                if (res.isError) {
                    if (res.isUsername) {
                        navigate(-1)
                        ...
                        </div>
                        {
                            item.levels ? (
                                <label htmlFor={item.name}>
                                    ...
                                    <select
                                        defaultValue={item.levels[0].value}
                                        id={item.name}
    ...
    const disabled = selectedSubjects.length === 0
                    ...
                    :
                        <>
                            ...
                            <label htmlFor="studyTime">
                                <span className="name-field">Ta'lim vaqti</span>
                                <select
                                    id="studyTime"
                                    className="input-fields "
                                    value={selectedTimeEdu}
                                    onChange={e => setSelectedTimeEdu(e.target.value)}
                                >
                                    {renderOptions(times)}
                                </select>
                            </label>
                            ...
                            <input disabled={disabled} className="input-submit" type="submit" value="Yuklash"/>
                        </>
                }
            </motion.form>
        </motion.div>
